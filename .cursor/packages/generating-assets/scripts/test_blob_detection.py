...
def detect_blobs(image_path):
    ...
    # Check alpha variance
    unique_alpha = np.unique(alpha)
    print(f"Unique alpha values: {unique_alpha}")
    
    mask = None
    if len(unique_alpha) == 1 and unique_alpha[0] == 255:
        print("Image is fully opaque. Attempting background color keying from (0,0).")
        bg_color = pixels[0, 0]
        print(f"Background color: {bg_color}")
        
        # Compare RGB channels
        diff = np.abs(pixels[:, :, :3] - bg_color[:3])
        # Sum of differences > tolerance (e.g. 30)
    ...
    for y, x in points:
        ...
        # Filter tiny noise
        if pixel_count > 50: # Increased threshold
            blobs.append({
...
def sort_blobs(blobs):
    ...
    if blobs:
        ...
        for b in blobs[1:]:
            ...
            # Simple row grouping: overlap in Y
            # If b center Y is "close" to row mean Y
            
            row_ys = [item['center'][1] for item in current_row]
            avg_y = sum(row_ys) / len(row_ys)
            
            # Use height of current blob as scale
            h = b['bbox'][3] - b['bbox'][1]
            
            # If center is within 0.5 * height of the average center
            if abs(b['center'][1] - avg_y) < (h * 0.6): 
